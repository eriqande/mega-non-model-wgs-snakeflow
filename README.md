...
I originally had made a scheme were we can start with one units.tsv file
that maybe only has six samples in it, and you can run that to
completion. Then you can update the units.tsv file to have two
additional samples in it, and that should then properly update the
genomics data bases. This was done by a system of writing Genomics_DBI
receipts that told us what was already in there.

I ended up never using that feature, and the receipts system complicated
the workflow and caused endless headaches during some reruns. So I
completely tossed it. You can’t add anything to the genomics databases,
now.
